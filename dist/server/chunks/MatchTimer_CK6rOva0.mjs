...
const missionBounds = {
  ...
  "14": { min: 0, max: 8 },
  "15": { min: 0, max: 6 }
  // precision_tokens: 0..6
};
const missionValueToPatch = (mission, value) => {
...
const missionValueFromMissionsFlat = (mission, missions) => {
  const m = missions && typeof missions === "object" ? missions : {};
  switch (mission) {
    case "1": {
    ...
    case "3-4": {
      ...
      const totalPoints = (m03_minecart ? 30 : 0) + (m03_bonus ? 10 : 0) + (m04_artifact ? 30 : 0) + (m04_support ? 10 : 0);
...
function MatchTimer() {
  ...
  return /* @__PURE__ */ jsxs("div", { className: "group flex items-center gap-4 bg-slate-900/50 backdrop-blur-xl px-5 py-2.5 rounded-2xl border border-slate-700/50 shadow-[0_8px_32px_rgba(0,0,0,0.4)] transition-all hover:border-slate-600", children: [
    /* @__PURE__ */ jsxs("div", { className: "relative", children: [
      /* @__PURE__ */ jsx(Clock, { className: `w-5 h-5 ${timer.isRunning ? "text-blue-400 animate-spin-slow" : "text-slate-600"}` }),
      timer.isRunning && /* @__PURE__ */ jsx("div", { className: "absolute inset-0 bg-blue-400/20 blur-md rounded-full animate-pulse" })
    ] }),
    /* @__PURE__ */ jsx("div", { className: `font-mono text-3xl font-black tabular-nums tracking-tighter transition-colors ${getStatusColor()}`, children: formatTime(timer.timeRemaining) }),
    /* @__PURE__ */ jsxs("div", { className: "flex flex-col border-l border-slate-700/50 pl-4 h-full justify-center", children: [
      /* @__PURE__ */ jsx("span", { className: "text-[9px] font-black text-slate-500 uppercase tracking-[0.2em] leading-none mb-1", children: "Status" }),
      /* @__PURE__ */ jsx("span", { className: `text-[10px] font-bold uppercase tracking-widest leading-none ${timer.isRunning ? "text-emerald-500" : "text-red-500"}`, children: timer.isRunning ? "Live" : "Stop" })
    ] })
  ] });
}

export { MatchTimer as M, missionValueFromMissionsFlat as a, missionValueToPatch as b, missionBounds as m };
